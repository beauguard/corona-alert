...
@IBDesignable open class TransitionButton: UIButton, UIViewControllerTransitioningDelegate, CAAnimationDelegate {
    ...
    private let springGoEase = CAMediaTimingFunction(controlPoints: 0.45, -0.36, 0.44, 0.92)
    ...
    open func stopAnimation(animationStyle: StopAnimationStyle = .normal, revertAfterDelay delay: TimeInterval = 1.0, completion: (() -> Void)? = nil) {
        ...
        case .expand:
            spiner.stopAnimation() // before animate the expand animation we need to hide the spiner first
            expand(completion: completion, revertDelay: delayToRevert) // scale the round button to fill the screen
        }
    }
